...
typedef std::tuple<ExprNode, bool> ExprResult;
class Parser {
public:
    Parser(Lexar* lexar);
    Program* parse();
    void      parseFunction(bool member = false, Struct parent = {});
    ...
    ExprNode   parseFuncCall(Func& func, Variable this_ptr = {type_infos.at("void")}, Variable return_address = {type_infos.at("void")});
    void parseHash();
